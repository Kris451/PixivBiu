...
│   ├── sysproxy/                 # Cross-platform OS system-proxy detection (macOS scutil / Windows registry / env vars), pure Go (no cgo)
...
│   ├── update/                   # Signed-feed checker (manifest.json + minisign verify) + Apply (download → verify SHA-256 → selfupdate binary swap → restart)
...
├── usr/                          # Gitignored; holds settings.json + state.json + downloads.json + cache/ (image cache, unless -cache-dir relocates it)
...
│   │   ├── lib/                  #   Stateless utilities — utils.ts (cn), icons.ts, format.ts (formatCount/hueFromId/formatBytes/formatDate/formatRelativeTime — source-tz-safe artwork date + locale-aware "n ago"), pixiv-image.ts (pximg → same-origin /api/v1/proxy/img rewrite), fetch-state.ts (FetchState<T>), url-params.ts (readPage/patchParams), poll.ts (pollUntil — restart + update-status catch-up), api/ (openapi-fetch client + `useApiErrorMessage` hook), theme/
...
├── .github/workflows/            # CI (push/PR) + Release & Docker (v* tag → GoReleaser + signed R2 feed / GHCR multi-arch)
...
| `make gen-winres` | Regenerate the Windows `.exe` resources (icon + version info) → gitignored `cmd/server/*.syso` |
...
- **Windows icon.** The Windows `.exe` carries an embedded icon (+ version-info) resource. `cmd/server/{versioninfo.json,icon.ico}` are the committed source; `go generate ./cmd/server` — wrapped as `make gen-winres` and run as a GoReleaser `before` hook — invokes the `goversioninfo` `go tool` (a `go.mod` tool directive, like `oapi-codegen`) to emit per-arch `cmd/server/resource_windows_<arch>.syso` (gitignored). The Go linker embeds only the `.syso` whose `GOOS/GOARCH` filename suffix matches the target, so non-Windows builds ignore it. The release binary is named `PixivBiu` (`.goreleaser.yaml` `binary:`); the in-app updater (`internal/update/apply.go::extractBinary`) locates the executable inside a downloaded archive by **matching the running binary's own base name** (`os.Executable`) against the archive's executable members (the execute-bit files in a tar.gz, the `.exe` files in a zip) — so a future rename can't break self-update, and an archive bundling several executables still resolves to the app binary. With no name match it falls back to the lone executable, and refuses (rather than guessing) when several are present but none matches.
...
- **Release** (`.github/workflows/release.yml`, on `v*` tag): [GoReleaser](https://goreleaser.com) (`.goreleaser.yaml`) builds the frontend (via `make build-web`), cross-compiles linux/macOS/windows × amd64/arm64 (`CGO_ENABLED=0`), and publishes archives + SHA-256 checksums + a grouped changelog (its commit range is chosen per channel — a stable aggregates its whole pre-release cycle) as a GitHub Release. A following workflow step then **dual-publishes to Cloudflare R2** — the source the in-app updater actually reads: it uploads the archives to an immutable `releases/<tag>/` path, then assembles, **minisign-signs**, uploads, and cache-purges a `manifest.json` feed (the GitHub Release stays as the human notes page + mirror). It **verifies the prior feed's signature before extending it**, so a tampered R2 `manifest.json` is discarded rather than re-signed with the CI key — the invariant that keeps "the signing key never leaves CI" meaningful even if the R2 write credentials leak. **Tags must be strict semver** (`v3.0.0`, prerelease `v3.0.0-beta.1`) — legacy `v2.x.ya/b` suffixes are rejected by GoReleaser. Dry-run with `goreleaser release --snapshot --clean`. Channels, the R2 feed/signing setup, the required secrets/keys, and the full tag→audience mapping live in [docs/RELEASE.md](docs/RELEASE.md).
...
- **Adding/editing a setting** = edit `internal/config/config.go`:
  ...
  3. Tag with `cfg:"category=...,enum=...|...,min=N,max=N,sensitive=true,restart=true,advanced=true,internal=true,hidden=true"` — the schema reflector picks this up. Both PATCH validation AND `GET /config/schema` come from these tags. The tag carries **no human-readable text** — the field's label/hint lives in the frontend `m.cfg_<key>` messages (`frontend/src/features/settings/i18n.ts`), so add one there (in all three locale JSONs); a field with no `cfg_*` message falls back to its raw dotted key in the UI. The `category` value is the **UI section id**, decoupled from the koanf path: several subsystems can share one (`server`/`log`/`inbox` → `category=system`) and a setting can ride under another section (`search.*` → `category=pixiv`). Reuse an existing section id where it fits; introducing a **new** one also needs a `SECTION_ICONS` entry (`frontend/src/features/settings/presentation.ts`) plus `settings_section_<id>` + `_desc` messages, or the section renders with the fallback gear icon and its raw id. Section order is derived from struct declaration order via `x-cfg-order` (each category sorts by its earliest field) — there is no manual order list to maintain.
...
- **Hidden-from-UI (`hidden=true`).** Fields tagged `hidden=true` (currently `pixiv.bypass_sni`) are **dropped from the reflected JSON Schema entirely** (`addLeaf` early-returns after registering the leaf, so no node and no `x-cfg-*` hint are emitted) — the settings UI never renders them, even under the "advanced" toggle. Unlike `internal`, `hidden` is a *UI-only* concern: the key stays in `schema.Fields`, so it remains fully PATCH-able and keyed-resettable through the API. The one cross-cut is `Reset(all)`, which preserves `internal` **or** `hidden` overrides — a blanket UI reset must not wipe a setting the user was never shown. Change it via PATCH, a `PIXIVBIU_*` env var, or hand-editing `settings.json`.
...
- **Path override**: `-config <path>` (default `./usr/settings.json`). If the file does not exist, defaults + env are used (no error); the file is created on the first successful PATCH. The **default** path — and the program-managed `pixiv.state_file` / `download.store_file` — are anchored to the **data root** via `runtimepath.Anchor(root, …)`, so the same binary reads/writes the same `usr/` regardless of launch CWD (matching `download.output_dir`). That root is `runtimepath.DataRoot(*dataDir)`: the `-data-dir` flag value, else `PIXIVBIU_DATA_DIR`, else `runtimepath.Root()` (the executable's directory; under `go run`/`make dev` it falls back to the repo root). The `-data-dir`/`PIXIVBIU_DATA_DIR` override (made absolute) relocates the whole `usr/`+default-downloads tree at once — the Electron desktop wrapper points it at the OS user-data dir so state lives outside the read-only `.app`. An **explicitly passed** `-config` keeps normal shell/CWD semantics, and absolute paths are always used as-is. The core itself never computes OS-specific dirs — it stays portable and only honors these overrides (the desktop shell owns OS placement). Two carve-outs ride the same precedence: the **image cache** has its own root via `runtimepath.CacheRoot(*cacheDir, root)` (`-cache-dir` flag, else `PIXIVBIU_CACHE_DIR`, else `usr/cache` under the data root) so a large, regenerable, machine-local cache can be parked in the OS cache dir instead of the backed-up/roaming app-data dir; and `log.file` (empty = stdout only) sends slog to a rotating file (the desktop points it at the OS logs dir, where a packaged app's stdout is otherwise invisible). A relative `log.file` anchors to the data root like `state_file`/`store_file`.
...
- **Schema**: All events (HTTP + non-HTTP) align to ECS (Elastic Common Schema). `httplog.SchemaECS.ReplaceAttr` is wired into the `slog.HandlerOptions` in `cmd/server/logging.go::newLogger`, so `time→@timestamp`, `level→log.level`, `msg→message`, `error→error.message` across all events.
...
- **Output sink**: stdout by default. Setting `log.file` (internal, restart) redirects slog to a **size-capped, rotating file** (lumberjack, 10 MB × 3 backups, 30 d) **instead of** stdout — for packaged desktop builds with no readable console (a relative path anchors to the data root; the desktop points it at the OS logs dir). `newLogger` fail-fasts at boot on a bad path (a directory, or an unwritable location) rather than letting logs silently vanish — lumberjack opens lazily and slog swallows write errors, so an unchecked bad path would pass boot with no logs anywhere. Separately, `main()` calls `suppressSIGPIPE()` (`signal.Notify(…, syscall.SIGPIPE)`) so a broken/closed **inherited stdout or stderr** can't terminate the server on a write to fd 1/2 (the boot banner, a `fatal:` print) — the Go runtime otherwise SIGPIPE-terminates on those. This is the process-wide fix for the desktop "dead parent pipe" case; a per-writer wrapper can't intercept the signal.
...
- **Numbered-pager loading feedback:** wrap the result block of every numbered-pager list in `<ListLoadingOverlay active={query.isPlaceholderData}>` (`components/list-loading-overlay.tsx`). `keepPreviousPage` keeps the prior page on screen with no skeleton during a page step, so on a slow fetch the UI looks frozen — the overlay dims the stale grid and floats a `LeapyLoading` pill at the top (gated by `useDelayedFlag`, so a fast/cached step never flashes). It pins to the top because the pagers `scrollAppToTop()` on a jump (the pager itself scrolls out of view). `isPlaceholderData` is the precise "showing the previous page while this one loads" signal; it's false for a cached-instant revalidation, so that doesn't dim. Used by `pages/search/results.tsx`, `pages/ranking/index.tsx`, `pages/user/index.tsx` (the last gates with `&& !isWalking` since the cursor walk already shows a skeleton). Most visible on the `bookmarks_desc`/`views_desc` search, which still fans out `search.sample.pages` upstream requests even after the `search.sample.concurrency` parallelization.
...
The key list, defaults, and validation live in `internal/config/config.go` — the `cfg:` struct tags are the **single source of truth** (defaults, `enum`/`min`/`max`, and the `sensitive`/`restart`/`advanced`/`internal`/`hidden` flags). At runtime, `GET /api/v1/config/schema` returns the always-fresh reflected JSON Schema, carrying `x-cfg-restart-required` / `x-cfg-advanced` / `x-cfg-internal` / `x-cfg-sensitive` hints for the settings UI (a `hidden` field is the exception — it's omitted from the schema entirely, so it carries no hint and never reaches the UI), plus `x-cfg-order` — a per-leaf declaration-order index the UI sorts by, so fields render in struct order rather than the alphabetical order the schema's `properties` map serializes to. Browse it there rather than maintaining a key list here; a static, human-readable enumeration of every `PIXIVBIU_*` variable (defaults, enums, and the `restart`/`internal`/`sensitive`/`advanced`/`hidden` flags) lives in [docs/CONFIGURATION.md](docs/CONFIGURATION.md). One exception to the static-defaults rule: `app.update.channel` has **no fixed default** — it's seeded at startup from the running build's maturity (`update.DefaultChannel(version)` → `config.SetDefaultUpdateChannel`, before `NewManager`), so a pre-release build defaults to its own channel; an explicit override still wins and, being build-derived, the key is exempt from diff-only default-pruning (see `pruneAgainstDefaults`). The five flag sets:

- **Restart-required** (`restart=true`) — persisted but applied only after `POST /config/restart`. The **PATCH-able** subset is `app.open_browser`, `log.format`, `pixiv.bypass_sni` (also `hidden`, so API/env/file only — not in the UI), `download.max_concurrent`; every other restart field is also `internal=true`, so it changes only via hand-edit + restart and never surfaces in `pending_restart`.
- **Internal / program-only** (`internal=true`): `server.*`, `log.file`, `pixiv.state_file`, `download.{referer,store_file}`, `inbox.*` — not writable through the runtime API/UI (PATCH + keyed reset rejected with 400; `reset {all:true}` preserves them; UI read-only). Edit by hand-editing `settings.json` or a `PIXIVBIU_*` env var. (`log.file` is also `restart=true` — like `state_file`/`store_file`, it's pinned at boot and changes only via hand-edit + restart, never surfacing in `pending_restart`.)
...
- **Hidden** (`hidden=true`): `pixiv.bypass_sni` — dropped from the reflected JSON Schema, so it never appears in the settings UI at all (not even under the "advanced" toggle). Still PATCH-able / keyed-resettable via the API; `reset {all:true}` preserves it. Change it via the API, env var, or hand-editing `settings.json`.
...
- `GET /auth/proxy/detect` — pre-login (unauthenticated) helper that reads the OS proxy config (macOS `scutil` / Windows registry) plus `HTTP(S)_PROXY`/`ALL_PROXY` env vars and returns address-only candidates, system-first (userinfo/credentials stripped; PAC-only setups yield none). It does **no network I/O and persists nothing** — onboarding tests a candidate via `/auth/connectivity`. `internal/sysproxy` owns the detection (pure Go, build-tag split per OS; parsers live in the shared file so they're testable anywhere).
...
- `GET /system/version` and `GET /system/update` (cached check result) are **open** (no auth) so the About panel renders pre-login; the mutating checks (`POST /system/update/check`, `.../apply`) require auth. `POST /system/update/apply` additionally requires the SPA's `X-PixivBiu-App` request header as a **CSRF guard** (a cross-origin page can't add it without a CORS preflight the server never grants → 403) and is **single-flighted** (a second concurrent apply → 409); it downloads/verifies/swaps synchronously (the check/apply writes opt out of the server write deadline), flushes `202 {status:"updating"}`, then `restart()`s. Dev / `go run` builds report `is_dev:true` and refuse apply. The check fetches a **minisign-signed `manifest.json`** from the Cloudflare R2 / CDN feed (not the GitHub API) and **verifies its signature before trusting any field** — a feed that fails verification is refused (`bad_request`/400), and the manifest's inline per-archive SHA-256 is what `Apply` verifies the download against (no separate `checksums.txt`). Update-feed reachability/download failures surface as `upstream_error`/502 with an **update-specific** message (not the Pixiv-outage copy). `release_notes` is **server-normalized for display** — `internal/update/checker.go::{aggregateNotes,sanitizeReleaseBody}` strip each release body's commit SHA, `(@author)`, and leading `## Changelog`, and an update spanning several versions is stitched newest-first under per-version `## <tag>` headings (frontend maps `h2`→version, `h3`→GoReleaser group); the SPA's `ReleaseNotesDialog` only renders it, while "View on GitHub" still points at the raw release (the dual-published GitHub Release). The `X-PixivBiu-App` header is sent on every request by the shared api client (`lib/api/client.ts`); keep its name in sync with `appRequestHeader` (`handler_system.go`).
...
- `upstream_error` / 502 — any other upstream failure (incl. update-feed reachability/download failures during an update check/apply, carrying an update-specific message).
...
- `image.cache.max_size_mb` (MB, `0` = unlimited) hot-reloads; the cache directory is `<cacheRoot>/img`, where `cacheRoot` defaults to `usr/cache` under the data root but is relocatable via `-cache-dir` / `PIXIVBIU_CACHE_DIR` (`runtimepath.CacheRoot`; the desktop shell points it at the OS cache dir). The fetch reuses `download.referer`, `pixiv.proxy`, and `download.http_timeout`.
...
- Docker images (releases are single self-contained binaries — see [Build & Release](#build--release); CI + tag-driven GitHub Releases and the signed Cloudflare R2 update feed are implemented)
